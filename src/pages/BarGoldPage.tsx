// path: gold/src/pages/BarGoldPage.tsx
...
export default function BarGoldPage() {
  ...
  const [form, setForm] = useState({
    date: new Date().toISOString(), firstname: "", lastname: "", idcard: "", address: "", phone: "",
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm({
      ...form,
      [name]: value,
      weightGram: name === "weightBaht" ? (parseFloat(value) * GOLD_BAHT_TO_GRAM_BAR).toFixed(2) : form.weightGram,
    });
  };

  const handleSubmit = async () => {
    try {
      const response = await fetch(`${API_BASE}/bar-gold/create`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, mode, weightBaht: parseFloat(form.weightBaht), amount: parseFloat(form.amount) }),
      });
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        maxWidth: 800,
        mx: "auto",
        bgcolor: theme.palette.background.paper,  // ✅ รองรับ Dark/Light Mode
      }}
    >
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={6}><TextField fullWidth label="น้ำหนักทอง (บาท)" name="weightBaht" value={form.weightBaht} onChange={handleChange} /></Grid>
        <Grid item xs={12} md={6}><TextField fullWidth label="น้ำหนักทอง (กรัม)" name="weightGram" value={form.weightGram} disabled /></Grid>
